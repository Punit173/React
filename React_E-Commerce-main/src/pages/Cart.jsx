...
import { addCart, delCart, setCart } from "../redux/action"; // Ensure setCart is defined
...
const Cart = () => {
  ...
  // Function to fetch and load user's cart from Firestore
  const loadUserCart = async (userId) => {
    const cartRef = doc(db, "carts", userId);
    const cartSnap = await getDoc(cartRef);

    if (cartSnap.exists()) {
      dispatch(setCart(cartSnap.data().items)); // Set cart items from Firestore
    }
  };
  ...
  const handleLogout = async () => {
    try {
      ...
      dispatch({ type: 'CLEARCART' }); // Clear the cart in local state
    } catch (error) {
  ...
  useEffect(() => {
    if (user) {
      loadUserCart(user.uid); // Load the user's cart when they log in
    } else {
      dispatch({ type: 'CLEARCART' }); // Clear cart if not logged in
    }
  }, [user, dispatch]);

  useEffect(() => {
    if (user) {
      saveUserCart(user.uid, state); // Save the cart to Firestore when cart state changes
    }
  }, [state, user]);
